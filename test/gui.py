...
current_position = 0
paused = False
selected_folder_path = ""  # Store the selected folder path for songs
...
def play_selected_song():
    ...
    if len(lbox.curselection()) > 0:
        ...
        with contextlib.closing(wave.open(full_path, 'r')) as f:
            ...
            pbar["maximum"] = duration  # Set the maximum value of the progress bar to the song duration
...
btn_select_music_folder = ctk.CTkButton(window, text="Select Music Folder",
                                  command=select_music_folder,
                                  font=("TkDefaultFont", 18))
btn_select_music_folder.pack(pady=10)
...
btn_play = ctk.CTkButton(btn_frame, text="Play", command=play_music, width=10,
                         font=("TkDefaultFont", 18))
btn_play.pack(side=tk.LEFT, padx=5)

# Create a button to pause the music
btn_pause = ctk.CTkButton(btn_frame, text="Pause", command=pause_music, width=10,
                          font=("TkDefaultFont", 18))
btn_pause.pack(side=tk.LEFT, padx=5)

# Create a button to go to the next song
btn_next = ctk.CTkButton(btn_frame, text=">", command=next_song, width=10,
                         font=("TkDefaultFont", 18))
